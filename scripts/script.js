...
function scrAnime() {
  ...
  elements.forEach(el => {
    // If the element is in the viewport (adjust for scroll)
    const rect = el.getBoundingClientRect();
    const inViewport = rect.top <= window.innerHeight && rect.bottom >= 0;

    if (inViewport) {
      // Only animate elements that are in the viewport
      el.classList.remove("scrollAnimation", "scrollAnimationTwo");

      if (currentScrollTop > lastScrollTop) {
        // Scroll Down
        el.classList.add("scrollAnimationTwo");
      } else {
        // Scroll Up
        el.classList.add("scrollAnimation");
      }
    }
  });
...
async function FetchDynaView(button) {
    ...
    try {
        ...
        // LEARM MORE BUTTON
        const hmBtnLki = document.getElementById("learnM1");
        const ctButLink = document.getElementById("headAbtLnk");
        const hmBtnLink = document.getElementById("headHmLnk");


if (hmBtnLki) {
hmBtnLki.addEventListener('click', () => {
    dynaContUrl = "https://alabiohio.github.io/LizzysUniquePalate/about.html"
    FetchDynaView(); // No scroll
    
                // Apply styles to the clicked button
            ctButLink.classList.add("clicked");
           hmBtnLink.classList.remove("clicked");
});
}     
         $(document).foundation();
        ...
        if (button && button.hasAttribute("data-value")) {
            ...
            switch (value) {
                ...
                case "mL3":
                    ...
                    break;
                case "mL4":
                    dynaContUrl = "https://alabiohio.github.io/LizzysUniquePalate/contact.html"
                    FetchDynaView(button); // Scroll left
                    closeMenu();
                    break;
                default:
...
/*const hmBtnLkiii = document.getElementById("learnM3").addEventListener('click', () => {
    dynaContUrl = "https://alabiohio.github.io/LizzysUniquePalate/gallery.html"
    FetchDynaView(); // No scroll
});
*/
